                            ...
                            <div class="bg-white border border-gray-200 rounded-lg p-6">
                                ...
                                <div class="grid grid-cols-2 gap-6">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-sm text-gray-900 mt-1">₱{{ formatCurrency(retentionAmount) }}</p>
                                    </div>
                                </div>
                            </div>
...
const approvePrebidding = async () => {
    const result = await Swal.fire({
        ...
        html: `
            <div class="text-left">
                ...
                <p class="text-sm text-gray-600 mt-3">Once approved, this will be sent back to CRM department for monitoring.</p>
            </div>
        `,
        icon: 'question',
    ...
    if (result.isConfirmed) {
        ...
        await axios.put(`/api/pending-approvals/finance/prebiddings/${route.params.id}/approve`)
        
        await Swal.fire({
            title: 'Sent to CRM!',
            text: 'Pre-bidding has been approved and forwarded to the CRM department.',
            icon: 'success',
            confirmButtonColor: '#0c8ce9',
...
const fetchPrebiddingDetails = async () => {
    try {
        ...
        const response = await axios.get(`/api/pending-approvals/finance/prebiddings/${id}`, {
